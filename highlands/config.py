...
# strip channel_binding param which psycopg2 doesn't support in URL
from urllib.parse import urlparse, urlencode, parse_qs, urlunparse
_raw_url = _raw_url.replace("postgres://", "postgresql://", 1)
if "channel_binding" in _raw_url:
    _p = urlparse(_raw_url)
    _qs = {k: v for k, v in parse_qs(_p.query).items() if k != "channel_binding"}
    _raw_url = urlunparse(_p._replace(query=urlencode(_qs, doseq=True)))
